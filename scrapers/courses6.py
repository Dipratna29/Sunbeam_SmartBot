...
class SunbeamDSAJavaCourseScraper:
    ...
    def scrape_to_txt(self, output_path="text_data/sunbeam_courses.txt"):
        ...
        # ================= COURSE HEADER =================
        lines.append("\n" + "=" * 90 + "\n")
        lines.append("COURSE: DATA STRUCTURES & ALGORITHMS USING JAVA\n")
        lines.append("=" * 90 + "\n\n")
        ...
        syllabus_btn = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, "//a[@href='#collapse123']"))
        )
        self.driver.execute_script("arguments[0].scrollIntoView(true);", syllabus_btn)
        syllabus_btn.click()

        syllabus_div = self.wait.until(
            EC.presence_of_element_located((By.ID, "collapse123"))
        )
        ...
        pre_btn = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, "//a[@href='#collapse124']"))
        )
        ...
        print("✅ DSA using Java course saved to text_data/sunbeam_courses.txt")
