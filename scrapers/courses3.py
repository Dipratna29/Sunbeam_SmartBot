...
class SunbeamDevOpsCourseScraper:
    ...
    def scrape_to_txt(self, output_path="text_data/sunbeam_courses.txt"):
        ...
        syllabus_btn = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, "//a[@href='#collapse48']"))
        )
        self.driver.execute_script("arguments[0].scrollIntoView(true);", syllabus_btn)
        syllabus_btn.click()

        syllabus_div = self.wait.until(
            EC.presence_of_element_located((By.ID, "collapse48"))
        )
        ...
        pre_btn = self.wait.until(
            EC.element_to_be_clickable((By.XPATH, "//a[@href='#collapse49']"))
        )
        ...
        print("✅ DevOps course saved to text_data/sunbeam_courses.txt")
